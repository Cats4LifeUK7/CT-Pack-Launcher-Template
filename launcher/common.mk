...
LIBDIRS := $(PORTLIBS) $(DEVKITPPC) $(DEVKITPPC)/powerpc-eabi \
...
LIBS :=	$(LIB_XMLXX) $(LIB_XML2) -lcurl -lwiisocket -lmbedtls -lmbedcrypto -lmbedx509 -lm -lpng -lz -lfat -lwiiuse -lbte -lasnd -logc -lvorbisidec -logg -lfreetype -lbz2 $(LIB_FILE) $(LIBS)
INCLUDES := $(SOURCE_DIR)/$(BUILD) $(SOURCE_DIR)/include $(LAUNCHER_DIR)/include $(LAUNCHER_DIR)/lib/libwiigui $(LAUNCHER_DIR)/lib \
						...
						$(PORTLIBS) $(DEVKITPPC) $(DEVKITPPC)/powerpc-eabi \
						$(DEVKITPRO)/libogc $(DEVKITPRO)/libogc/lib/wii \
						$(CURDIR)

CFLAGS		=	-fdata-sections -ffunction-sections -g -O2 -O3 -Wextra -Wall -Wno-deprecated-declarations -Wno-multistatement-macros $(MACHDEP) $(INCLUDE)
CXXFLAGS	=	-Xassembler -aln=$@.lst $(CFLAGS)
LDFLAGS		=	-g $(MACHDEP) -Wl,--gc-sections -Wl,-Map,$(notdir $@).map,--section-start,.init=$(INIT_ADDR) -T $(LAUNCHER_DIR)/lib/rvl.ld
